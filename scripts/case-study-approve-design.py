#!/usr/bin/env python3
"""Record design approval on the fed package (Sally gate)."""
...
def main() -> int:
    ...
    parser.add_argument("--package", type=Path, help="Package JSON (default: scripts/case-study-packages/{slug}.json)")
    ...
    pkg = load_json(path)
    pkg.setdefault("approvals", {}).setdefault("design", {})
    pkg["approvals"]["design"] = {
        "status": "approved",
        "approved_by": args.by,
        "approved_at": datetime.now(timezone.utc).isoformat(),
    }
    path.write_text(json.dumps(pkg, indent=2) + "\n", encoding="utf-8")
    if path != pkg_path and pkg_path.is_file():
        pkg_path.write_text(json.dumps(pkg, indent=2) + "\n", encoding="utf-8")
    print(f"Design approved on {path.relative_to(ROOT)}")
    return 0
